...
Approval mode: OpenClaw cron drafts candidates every two hours and sends them to Telegram for approval. ליאור wants future Poanta approval requests and Poanta update messages sent in the shared Poanta group, not his private chat, once that Telegram group chat id is available. GitHub Actions publishing is manual-only for now.
...
What it does:
- pulls approved RSS-only sources from `rss_sources.json`
...
Current caveats:
- RSS-only phase: no homepage scraping and no fallback readers. Sources without clean RSS stay excluded until a real RSS/API endpoint is found.
- public views/comments are not consistently exposed by RSS, so v1 uses source freshness + headline signal as proxy
