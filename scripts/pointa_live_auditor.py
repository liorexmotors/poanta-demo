...
def iran_deal_decision_tokens(item: dict[str, Any]) -> set[str]:
    ...
    has_decision_delay = bool(re.search(
        r"דחה|לא החליט|בלי החלטה|ללא הכרעה|לא קיבל החלטה|בלי מסר ברור|הסתיימה פגישת|סיימו דיון|חדר המצב|הכרעה|אישור|קרובים להבנות|מחלוקות|כספים מוקפאים|שחרור הכספים|אורניום מועשר|הורמוז",
        text,
    ))
